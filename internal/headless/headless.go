...
func Run(ctx context.Context, opts Options, deps Deps, stdout, stderr io.Writer) error {
	...
	cacheRetention, cacheTTLSeconds, _, cacheWarm, cacheWarmTurn := deps.Config.CacheForProvider(deps.Provider.Name())
	...
	runner := agent.New(agent.Config{
		...
		DistillThresholdPercent: deps.Config.DistillThresholdPercentFor(deps.Provider.Name(), deps.ModelID),
		...
		PassbackReasoning:       deps.Config.PassbackReasoningFor(deps.Provider.Name()),
		MaxToolResultBytes:      deps.Config.CacheMaxToolResultBytes,
		SpillBytes:              deps.Config.CacheSpillBytes,
